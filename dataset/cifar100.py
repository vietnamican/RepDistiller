...
class CIFAR100InstanceSampleNoPositive(CIFAR100BackCompat):
    """
    CIFAR100Instance+Sample Dataset
    """
    def __init__(self, root, train=True,
    ...
    def __getitem__(self, index):
        ...
        if not self.is_sample:
            ...
            # sample_idx = np.hstack((np.asarray([pos_idx]), neg_idx))
            # sample_idx = 
            return img, target, index, neg_idx
...
def get_cifar100_dataloaders_sample_no_postive(batch_size=128, num_workers=8, k=4096, mode='exact',
                                    is_sample=True, percent=1.0):
    """
    cifar 100
    """
    ...
    train_set = CIFAR100InstanceSampleNoPositive(root=data_folder,
                                       download=True,
                                       train=True,
                                       transform=train_transform,
                                       k=k,
